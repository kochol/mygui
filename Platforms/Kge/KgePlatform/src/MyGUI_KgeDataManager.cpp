...
namespace MyGUI
{
	...
	KgeDataManager::KgeDataManager() :
		mIsInitialise(false)
	{
	}

	void KgeDataManager::initialise()
	{
	...
	IDataStream* KgeDataManager::getData(const std::string& _name)
	{
	...
	void KgeDataManager::freeData(IDataStream* _data)
	{
		delete _data;
	}

	bool KgeDataManager::isDataExist(const std::string& _name)
	{
		const VectorString& files = getDataListNames(_name);
		return !files.empty();
	}

	const VectorString& KgeDataManager::getDataListNames(const std::string& _pattern)
	{
	...
	const std::string& KgeDataManager::getDataPath(const std::string& _name)
	{
	...
	void KgeDataManager::addResourceLocation(const std::string& _name, bool _recursive)
	{
		ArhivInfo info;
